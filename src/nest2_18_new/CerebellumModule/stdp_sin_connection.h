...
public:
   ...
   */
  void send( nest::Event& e, nest::thread t, const nest::CommonSynapseProperties& cp );
  ...
  const nest::CommonSynapseProperties& )
{
  ...
  e.set_delay_steps( get_delay_steps() );  //TODO: set_delay does not exists in NEST 2.18, but I am no sure I am changing the expected behavior
